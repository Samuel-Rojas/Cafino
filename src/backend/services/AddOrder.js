...
/**
 * Deletes a single coffee order/entry by ID.
 *
 * @param {string} orderId - UUID of the coffee entry to delete
...
export async function deleteOrder(orderId) {
  try {
    if (!orderId || orderId.trim() === "") {
      return {
        success: false,
        error: "Order ID is required to delete a coffee order",
      };
    }
    ...
    if (error) {
      ...
      return {
        success: false,
        error: error.message || "Failed to delete coffee order",
      };
    }
    ...
    console.error("Unexpected error deleting coffee order:", error);
    return {
      success: false,
      error:
        error.message ||
